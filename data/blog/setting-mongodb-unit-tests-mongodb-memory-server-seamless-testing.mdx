...
```js
// Check the current major versions on npm before you start — both
// mongodb-memory-server and mongoose move fast enough that a version
// pinned in a blog post goes stale within a year. The API shown here
// has been stable across recent major versions of both.
const mongoose = require('mongoose')
...
const connect = async () => {
  ...
  // Note: useNewUrlParser and useUnifiedTopology are deprecated no-ops in
  // Mongoose 6+, so no extra connection options are needed here.
...
```js
const mongoose = require('../src/provider/mongodb')
...
### Pin the MongoDB Binary Version

By default, `mongodb-memory-server` downloads a MongoDB binary the first time it runs and caches it locally. Left unpinned, that can mean your tests run against a different MongoDB version than production — and in CI, the first download on a cold cache adds real time and a new failure mode (the download itself can flake). Pin it explicitly to match your production MongoDB version:

```js
mongod = await MongoMemoryServer.create({
  binary: {
    version: '7.0.14', // match whatever your production MongoDB actually runs
  },
})
```

Check your production MongoDB version and use that here — the point isn't the specific number, it's that the test database and the production database should be running the same major version so you're not debugging a behavior difference that only exists in tests.

### Using a Replica Set for Transactions

If your application code uses MongoDB transactions (`session.startTransaction()`), a plain `MongoMemoryServer` won't work — **transactions require a replica set**, and a standalone in-memory instance doesn't provide one. This is one of the most common "why does this only fail in tests" surprises with this library. Swap in `MongoMemoryReplSet` instead:
...
A single-member replica set (`count: 1`) is enough to unlock transaction support in tests — you don't need to simulate a full multi-node cluster to test transactional code paths.

### Sharing One Instance Across Your Whole Test Suite

The setup above starts and stops a fresh in-memory MongoDB per test _file_, which is simple but adds startup overhead if you have many files. For larger suites, start one instance for the entire run using your test runner's global setup/teardown hooks. With Jest:
...
Each test file then connects to `process.env.MONGO_URI` and is responsible for its own `clearDatabase()` between tests (as shown earlier), rather than each file paying the cost of spinning up a new MongoDB binary. This same pattern works with Vitest too — the `connect`/`closeDatabase`/`clearDatabase` functions shown throughout this post don't depend on anything Jest-specific, since they're just Mongoose calls wrapped in plain async functions.
